package com.betterraffle.main.helper;

public class Constants {

    public static final String BASE_RESOURCE_PATH = "./resources";
    public static final String INPUT_DIR_PATH = "/input";
    public static final String OUTPUT_DIR_PATH = "/output";
    public static final String IMAGES_DIR_PATH = "/images";


    public static final String CLASSIFIER_ALT = "/haarcascade_frontalface_alt.xml";
    public static final String CLASSIFIER_ALT_TREE = "/haarcascade_frontalface_alt_tree.xml";

    public static final String TRAINING_HELPER_PATH = "/training_helper.txt";

    public static final String END_OF_FILE = "--end--";

    public static final int RUN_MODE_EXTRACT = 0;
    public static final int RUN_MODE_MAIN = 1;

    public static final String EXT_JPG = ".jpg";
    public static final String EXT_DS = ".ds_store";

    public static final String TABLE_NAME = "user";

    public static final int COL_ID_INDEX = 1;
    public static final int COL_NAME_INDEX = 2;
    public static final int COL_PHONE_INDEX = 3;
    public static final int COL_TOKEN_INDEX = 4;
    public static final int COL_LAST_WON_INDEX = 5;

    public static final String COL_ID = "UserId";
    public static final String COL_NAME = "UserName";
    public static final String COL_PHONE = "Phone";
    public static final String COL_TOKEN = "Token";
    public static final String COL_LAST_WON = "LastWon";

    public static final int START_TOKEN = 9944000;
}
